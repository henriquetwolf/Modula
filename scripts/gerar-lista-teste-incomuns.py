"""Gera uma segunda planilha de teste: primeiros nomes incomuns e quantidade
variavel de sobrenomes (15 com 1, 15 com 2 e 15 com 3), sendo 40% feminino.

Os CPFs sao validos e nao repetem os da planilha lista_teste_cpf.xlsx, para as
duas listas poderem ser importadas no mesmo tenant sem colisao.
"""
...
ARQUIVO_SAIDA = RAIZ / "lista_teste_nomes_incomuns.xlsx"
ARQUIVO_ANTERIOR = RAIZ / "lista_teste_cpf.xlsx"

# 15 pessoas para cada quantidade de sobrenomes
POR_GRUPO = 15
QUANTIDADES_SOBRENOMES = (1, 2, 3)
PROPORCAO_FEMININO = 0.40
SEED = 20260908

# Primeiros nomes incomuns. Sobrenomes com particula ("de", "da") ficaram de
# fora de proposito para a contagem de sobrenomes continuar exata.
...
PRIMEIROS_NOMES_MASCULINOS = [
    "Abelardo", "Anacleto", "Belarmino", "Bonifácio", "Cleanto", "Crisóstomo",
    "Demóstenes", "Diógenes", "Eleutério", "Epaminondas", "Firmino",
    "Fulgêncio", "Genésio", "Gumercindo", "Hermenegildo", "Higino",
    "Idelfonso", "Isaltino", "Jerônimo", "Leôncio", "Melquíades",
    "Nepomuceno", "Norberto", "Onofre", "Pantaleão", "Policarpo", "Querubino",
    "Quirino", "Saturnino", "Sinval", "Teodomiro", "Tertuliano", "Ubirajara",
    "Venâncio", "Wenceslau", "Zeferino",
]

SOBRENOMES = [
    "Abreu", "Aguiar", "Albuquerque", "Amaral", "Antunes", "Aragão",
    "Assunção", "Barreto", "Bastos", "Bittencourt", "Braga", "Brandão",
    "Bulhões", "Camargo", "Campelo", "Carneiro", "Cerqueira", "Chagas",
    "Coelho", "Cordeiro", "Cunha", "Delgado", "Drummond", "Escobar",
    "Esteves", "Falcão", "Farias", "Figueiredo", "Fontoura", "Furtado",
    "Galvão", "Guimarães", "Holanda", "Jucá", "Lacerda", "Leitão", "Macedo",
    "Magalhães", "Marinho", "Meireles", "Modesto", "Mourão", "Novaes",
    "Peixoto", "Pontes", "Quadros", "Rezende", "Sarmento", "Seixas",
    "Tourinho", "Valadares", "Vasconcelos", "Veríssimo", "Wanderley",
    "Xavier", "Zamboni",
]
...
def cpfs_da_planilha_anterior() -> set[str]:
    """CPFs (so digitos) ja usados na primeira planilha, para nao repetir."""
    if not ARQUIVO_ANTERIOR.exists():
        return set()

    planilha = load_workbook(ARQUIVO_ANTERIOR, read_only=True).active
    usados = set()
...
def main() -> None:
    ...
    if qtd_feminino * len(QUANTIDADES_SOBRENOMES) > len(femininos):
        raise SystemExit("Faltam primeiros nomes femininos incomuns.")
    if qtd_masculino * len(QUANTIDADES_SOBRENOMES) > len(masculinos):
        raise SystemExit("Faltam primeiros nomes masculinos incomuns.")

    linhas: list[tuple[str, str, int, str]] = []
    cpfs_usados = cpfs_da_planilha_anterior()
